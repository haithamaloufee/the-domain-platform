...
Sprint 12A adds runtime-validated server helpers for `/api/public/homepage`, `/api/public/statistics`, and `/api/public/partners`, plus shared public/admin CMS types. The visual homepage still uses the local content adapter in this backend-foundation sprint; Sprint 12B will add the protected admin UI/BFF and a later public wiring step can replace local values only when published/verified backend content exists. No fallback statistics or partners are fabricated by the API.
