...
Copy this app's `.env.example` to `.env.local` to target the local API at `http://localhost:5000`. Sprint 6A does not connect the login form; authentication integration is the next sprint. Never put signing keys or passwords in a `NEXT_PUBLIC_` variable.
