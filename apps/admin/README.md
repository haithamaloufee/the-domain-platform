...
To test locally, provision the SuperAdmin through the Sprint 6A setup, configure and enable Cloudinary in the backend environment, start the backend on port 5000, then start admin on port 3001 and sign in at `/login`. Upload a small supported file as Draft, verify it appears in the paginated library, edit its metadata, assign it to an event, and exercise approval/hide transitions. No production media belongs in this repository. Dashboard metrics, password management, user administration, and full event-gallery ordering remain out of scope.
